...
export const fetchLocation = async (draftMode = false) => {
  // Always fetch all findingUsSettings docs (including drafts)
  const query = `*[_type == 'findingUsSettings']{_id, content, towns, directions}`;
  ...
  // Return the first (only) findingUsSettings doc, or null
