...
export const fetchHero = async (draftMode = false) => {
  // Always fetch all heroSettings docs (including drafts)
  const query = `*[_type == "heroSettings"]{_id, content, backgroundImage}`;
  ...
  // Return the first (only) heroSettings doc, or null
