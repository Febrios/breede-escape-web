    ...
    return (
        <section className="activities py-20 sm:py-32 bg-[var(--cream)]" id="activities">
            <div className="section-inner max-w-[1600px] mx-auto px-[3vw]">
                ...
                {Array.from({ length: Math.ceil(activities.length / 4) }).map((_, groupIdx) => (
                    <div
                        key={groupIdx}
                        className={`activities-grid grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 border-b border-[rgba(26,46,26,0.1)] ${groupIdx === 0 ? 'mt-10 sm:mt-16' : ''} gap-4 sm:gap-0`}
                    >
                        {activities.slice(groupIdx * 4, groupIdx * 4 + 4).map((act: any, idx: number) => (
