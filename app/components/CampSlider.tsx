...
export default function CampSlider({ images, interval = 3500 }: CampSliderProps) {
    ...
    const validImages = images.filter(img => !brokenUrls.includes(img.url));
    ...
    }, [current, validImages.length, interval]);
    ...
    return (
        <>
            <div className="relative w-full h-[200px] sm:h-[250px] overflow-hidden rounded-md">
                {validImages.map((img, i) => (
                    <Image
                        ...
                        onError={() => setBrokenUrls(urls => urls.includes(img.url) ? urls : [...urls, img.url])}
                    />
                ))}
            ...
            {modalOpen && validImages[modalIndex] && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setModalOpen(false)}>
                    ...
                    <button
                        ...
                        onClick={e => { e.stopPropagation(); setModalIndex((modalIndex - 1 + validImages.length) % validImages.length); }}
                    ...
                    <button
                        ...
                        onClick={e => { e.stopPropagation(); setModalIndex((modalIndex + 1) % validImages.length); }}
