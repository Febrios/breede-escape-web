...
export default async function Camps() {
    ...
    return (
        <section className="camps py-32 bg-[var(--forest)]" id="camps">
            <div className="section-inner max-w-[1200px] mx-auto px-[5vw]">
                ...
                <div className="camps-grid grid grid-cols-1 md:grid-cols-2 gap-2 mt-16">
                    {camps.map((camp: any, idx: number) => (
                        <div key={camp._id} className="camp-card relative overflow-hidden h-[560px] cursor-pointer group">
                            <Image
                                src={camp.imageUrl || "https://via.placeholder.com/900x560"}
                                alt={camp.name}
                                width={900}
                                height={560}
                                className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 group-hover:saturate-100 group-hover:brightness-75 saturate-80 brightness-90"
                            />
                            <div className="camp-overlay absolute inset-0 bg-gradient-to-t from-[rgba(15,26,15,0.92)] via-[rgba(15,26,15,0.3)] to-transparent p-10 flex flex-col justify-end">
                                ...
                                <p className="camp-desc text-[0.9rem] leading-[1.7] text-[rgba(245,240,232,0.75)] font-light mb-6 max-w-[340px]">{camp.description}</p>
